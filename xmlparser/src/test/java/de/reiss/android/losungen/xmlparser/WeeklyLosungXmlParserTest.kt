...
class WeeklyLosungXmlParserTest {
    ...
    private fun check(fileName: String,
        ...
        parsed.forEach {
            assertEquals(year, Calendar.getInstance().apply { time = it.StartDatum }.get(Calendar.YEAR))
            assertEquals(year, Calendar.getInstance().apply { time = it.EndDatum }.get(Calendar.YEAR))
